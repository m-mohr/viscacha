...
function writeBody() {
	...
	for( var txtid = 0; txtid < this.textInputs.length; txtid++ ) {	
		...
		if( wordtxt ) {			
			...
			d.writeln( '<div class="spellchecktext">' );
...
function printForHtml( n ) {
	...
	if( htmlstr.length == 1 ) {
		...
		htmlstr = htmlstr.replace( /\[/g, '&#91' );
		htmlstr = htmlstr.replace( /\]/g, '&#93' );
...
function _wordInputStr( word ) {
	var str = '<input readonly="readonly" class="transparent" type="text" value="' + word + '" size="' + word.length + '" />';
	return str;
}
