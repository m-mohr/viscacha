...
class tar {
    ...
    function tar() {
        ...
        {
            $this->current_dir = $_SERVER['DOCUMENT_ROOT'];
        }
        else
        {
            $this->current_dir = './';
    ...
    function new_tar($tarpath, $tarname, $wrapper = "") {
        
        $this->tarfile_name = $tarname;
        $this->tarfile_path = $tarpath;
        if($wrapper !== "")
            $this->tarfile_wrap = $wrapper."://";
    ...
    function write_tar() {
        ...
        $FH = fopen( $this->tarfile_wrap.$this->tarfile_path_name, 'wb' );
