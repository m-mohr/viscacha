 ...
 * path for PDF fonts
 */
define ("FPDF_FONTPATH", $config['fpath']."/classes/fpdf/font/");

/**
 * cache directory for temporary files (full path)
 */
define ("K_PATH_CACHE", $config['fpath']."temp/pdfimages/");

/**
 * cache directory for temporary files (url path)
 */
define ("K_PATH_URL_CACHE", $config['furl']."temp/pdfimages/");
	...
	class TCPDF {
		...
		*/
		...
		function SetFixedFont($family) {
			...
			// try to add font (if not already added)
			if($this->isunicode) {
				$this->AddFont($family, $this->FontStyle);
			}

			//Test if font is already selected
			if(($this->FontFamilyMono == $family)) {
				return;
			}

			$fontkey = $family.$this->FontStyle;
			...
			if(!isset($this->fonts[$fontkey])) {
				...
				if(isset($this->CoreFonts[$fontkey])) {
					...
					$this->Error('Undefined font: '.$family.' '.$this->FontStyle);
				}
			}
			//Select it
			$this->FontFamilyMono = $family;
		}
		...
		*/
		...
		function writeHTML($html, $ln=true, $fill=0) {
			...
			$html=strip_tags($html,"<h1><h2><h3><h4><h5><h6><b><u><i><a><img><p><br><br/><strong><em><code><tt><font><blockquote><li><ul><ol><hr><td><th><tr><table><sup><sub><small>"); //remove all unsupported tags
...
function html_entity_decode_php4($text_to_convert) {
	...
	$return_text = preg_replace('~&#0*([0-9]+);~e', 'code_to_utf8(\\1)', $return_text);
	return $return_text;
}
