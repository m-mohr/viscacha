...
class Jabber
{
	...
	function Connect()
	{
		...
		{
			// MOD: character set variable
			$this->SendPacket("<?xml version='1.0' encoding='" . $this->charset . "' ?" . ">\n");
			$this->SendPacket("<stream:stream to='{$this->server}' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams'>\n");

			// MOD: changed this to use the variable
			sleep($this->connect_sleep_timer);
	...
	function SendMessage($to, $type = "normal", $id = NULL, $content = NULL, $payload = NULL)
	{
		if ($to && is_array($content))
		{
			...
			{
				// MOD: added htmlspecialchars()
				$xml .= "<subject>" . htmlspecialchars($content['subject']) . "</subject>\n";
			}
			...
			{
				// MOD: added htmlspecialchars()
				$xml .= "<thread>" . htmlspecialchars($content['thread']) . "</thread>\n";
			}

			// MOD: added htmlspecialchars()
			$xml .= "<body>" . htmlspecialchars($content['body']) . "</body>\n";
			$xml .= $payload;
			$xml .= "</message>\n";
	...
	function GetFromQueueByAttr($packet_type, $attr, $value, $partial = false)
	{
		$found_message = FALSE;
		$valuelen = strlen($value);

		foreach ($this->packet_queue as $key => $packet)
		{
			if ($partial)
				$usethis = (substr($packet[$packet_type]['@'][$attr], 0, $valuelen) == $value);
			else
				$usethis = ($packet[$packet_type]['@'][$attr] == $value);

			if ($usethis)
			{
				$found_message = $packet;
				unset($this->packet_queue[$key]);
	...
	function _sendauth_digest()
	{
		...
		if (viscacha_function_exists('mhash')) {
			...
			$mh = sha1($this->stream_id . $this->password);
		}
		$payload = "<username>{$this->username}</username>
					<resource>{$this->resource}</resource>
					// MOD: Changed this
...
{
	// MOD: @'ed all socket functions so they are silent
	var $active_socket;
