...
function fetch_rss ($url, $maxage = 3600) {
    // initialize constants
    init();

    if ( !isset($url) ) {
        error2("fetch_rss called without a url");
        return false;
    }
    ...
    if ( !MAGPIE_CACHE_ON ) {
        ...
        if ( is_success( $resp->status ) ) {
            ...
            error2("Failed to fetch $url and cache is off");
            return false;
        }
    }
    // else cache is ON
        ...
        $cache = new RSSCache( MAGPIE_CACHE_DIR, $maxage );
...
function error2 ($errormsg, $lvl=E_USER_WARNING) {
        global $MAGPIE_ERROR;
...
function init () {
    ...
    if ( !defined('MAGPIE_CACHE_DIR') ) {
        define('MAGPIE_CACHE_DIR', './cache/grabrss/');
    }
    ...
    if ( !defined('MAGPIE_FETCH_TIME_OUT') ) {
        define('MAGPIE_FETCH_TIME_OUT', 8); // 5 second timeout
    }
