...
class Ftp {
	...
	public static function create($verbose = false, $localEcho = false) {
		if (extension_loaded('ftp')) {
			return new FtpExtension($verbose, $localEcho);
		} else {
			$mod_sockets = true;
			...
			if ($mod_sockets == true) {
				return new FtpSockets($verbose, $localEcho);
			} elseif ($mod_sockets == false && function_exists('fsockopen')) {
				return new FtpNative($verbose, $localEcho);
			}
		}
		return null;
	}
...
//	Parses 1 line like:		"drwxrwx---  2 owner group 4096 Apr 23 14:57 text"
	...
	function connect($server = NULL) {
		...
		if (!$syst)
			...
			if (preg_match("/win|dos|novell/iu", $syst[0]))
				$this->OS_remote = FTP_OS_Windows;
			elseif (preg_match("/os/iu", $syst[0]))
				$this->OS_remote = FTP_OS_Mac;
			elseif (preg_match("/(li|u)nix/iu", $syst[0]))
				$this->OS_remote = FTP_OS_Unix;
			else
				$this->OS_remote = FTP_OS_Mac;
