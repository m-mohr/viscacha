...
function sort_dirlist($a, $b) {
	$ai = \Str::substr_count($a, '/');
	$bi = \Str::substr_count($b, '/');
	if ($ai  == $bi) {
		$test = array($a, $b);
		sort($test);
		return ($test[0] == $a) ? -1 : 1;
...
function dir_array($dir, $chop = false) {
	$array = array();
	$d = dir($dir);
	if ($chop != false && \Str::substr($dir, -1, 1) != '/') {
		$dir .= '/';
	}
	if ($chop != false && \Str::substr($chop, -1, 1) != '/') {
		$chop .= '/';
	}
...
function createParentDir($parentfile, $path) {
	global $filesystem;
	$parents = array();
	while(($pos = \Str::lastIndexOf($parentfile, DIRECTORY_SEPARATOR)) !== false) {
		$parentfile = \Str::substr($parentfile, 0, $pos);
		$parents[] = $parentfile;
	}
