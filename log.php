...
elseif ($_GET['action'] == "pwremind2") {
	...
	$user = $db->fetch("SELECT id, name, mail, pw FROM {$db->pre}user WHERE mail = '{$_POST['email']}' AND deleted_at IS NULL");
	if (!$user) {
...
elseif ($_GET['action'] == "pwremind3") {
	...
	$user = $db->fetch("SELECT id, pw, mail, name FROM {$db->pre}user WHERE id = '{$_GET['id']}' AND deleted_at IS NULL");
	if (!$user) {
		error($lang->phrase('log_pwremind_failed'), "log.php?action=pwremind".SID2URL_x);
	}
	else {
